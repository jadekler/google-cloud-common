...
var testcases = pb.Testcases{
	SignedV4Tests: []*pb.SignedV4Test{
...

		{
			...
			Method: "POST",
			Expiration: 10,
			Headers: map[string]*pb.ValueList{
				"M-goog-resumable": {Values: []string{"start"}},
			},
			Timestamp: "20190201T090000Z",
...

		{
			...
			Object: "test-object",
			Headers: map[string]*pb.ValueList{
				"foo": {Values: []string{"foo-value"}},
				"BAR": {Values: []string{"BAR-value"}},
			},
			Method: "GET",
			Expiration: 10,
			Timestamp: "20190201T090000Z",
		...
		// this test or perform the concatenation of header values themselves.
		{
			...
			Object: "test-object",
			Headers: map[string]*pb.ValueList{
				"foo": {Values: []string{"foo-value1", "foo-value2"}},
				"bar": {Values: []string{"bar-value1", "bar-value2"}},
			},
			Method: "GET",
			Expiration: 10,
			Timestamp: "20190201T090000Z",
...

		{
			...
			Object: "test-object",
			Headers: map[string]*pb.ValueList{
				"leading": {Values: []string{"    xyz"}},
				"trailing": {Values: []string{"abc    "}},
				"collapsed": {Values: []string{"abc    def"}},
			},
			Method: "GET",
			Expiration: 10,
			Timestamp: "20190201T090000Z",
		...
		// Separated from "Headers should be trimmed" test so it can be skipped on single-header-value-only platforms.
		{
			...
			Object: "test-object",
			Headers: map[string]*pb.ValueList{
				"foo": {Values: []string{"  abc  ", "  def  ", "  ghi  jkl  "}},
			},
			Method: "GET",
			Expiration: 10,
			Timestamp: "20190201T090000Z",
		...
		// Headers associated with customer-supplied encryption keys should not be included in the signature
		{
			...
			Object: "test-object",
			Headers: map[string]*pb.ValueList{
				"X-Goog-Encryption-Key": {Values: []string{"ignored"}},
				"X-Goog-Encryption-Key-Sha256": {Values: []string{"ignored"}},
				"X-Goog-Encryption-Algorithm": {Values: []string{"ignored"}},
			},
			Method: "GET",
			Expiration: 10,
			Timestamp: "20190201T090000Z",
...
func main() {
	...
	f, err := os.Create("testcases.json")
	if err != nil {
		panic(err)
	}
	defer f.Close()

	if err := m.Marshal(f, &testcases); err != nil {
		panic(err)
	}
}
